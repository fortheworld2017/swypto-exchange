  ...
  Transaction = sequelize.define "Transaction",
    ...
    ,
      ...
      classMethods:
        ...
        findPendingByUser: (userId, callback)->
          query =
            where:
              user_id: userId
              category: MarketHelper.getTransactionCategory "receive"
        ...
        findProcessedByUserAndStatus: (userId, status, callback)->
          query =
            where:
              user_id: userId
#              status: status
          ...
          query.where = sequelize.and(query.where, sequelize.or({category: MarketHelper.getTransactionCategory("receive"), balance_loaded: status}, {category: MarketHelper.getTransactionCategory("send")}))
          Transaction.findAll(query).complete callback
